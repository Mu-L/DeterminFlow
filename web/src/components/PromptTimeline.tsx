...
export default function PromptTimeline({ history }: PromptTimelineProps) {
  return (
    <div className="rounded-lg bg-slate-800/80 border border-slate-700/50 p-4" role="region" aria-label="提示词进化时间线">
...
function TimelineEntry({ entry }: { entry: PromptHistoryEntry }) {
  ...
  return (
    <div className="relative pb-4" role="article" aria-label={`版本 ${entry.version} 到 ${entry.version + 1} 的变更`}>
      {/* Node dot */}
      <div className="absolute -left-3.5 h-3 w-3 rounded-full bg-slate-900 border-2 border-indigo-500" aria-hidden="true" />
